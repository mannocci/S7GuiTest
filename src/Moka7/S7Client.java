...
private void SendPacket(byte[] Buffer, int Len)
{
    ...
    int tpktLen = S7.GetWordAt(Buffer, 2);

    System.out.printf(
        "SEND: requested=%d TPKT=%d socketClosed=%s%n",
        Len,
        tpktLen,
        TCPSocket == null ? "null" : TCPSocket.isClosed()
    );

    if (Len != tpktLen) {
        System.err.printf(
            "*** ERRORE: Len=%d ma TPKT dichiara %d ***%n",
            Len, tpktLen
        );
    }
...
public int Connect()
{
    ...
    if (!Connected)
    {
        ...
        TCPConnect();
        System.out.printf("TCP result = 0x%08X%n", LastError);

        if (LastError == 0)
        {
            System.out.println("=== ISOConnect ===");
            ISOConnect();
            System.out.printf("ISO result = 0x%08X%n", LastError);

            if (LastError == 0)
            {
                System.out.println("=== NegotiatePduLength ===");
                LastError = NegotiatePduLength();
                System.out.printf("PDU result = 0x%08X%n", LastError);
            }
        }
    }
    ...
    System.out.println("Connected finale = " + Connected);
...
//        return 0;
//
//    // Garantisce uno stato iniziale pulito
//    CloseConnection();
//
//    LastError = 0;
//
...
//    }
    ...
    synchronized public  int ReadArea(int Area, int DBNumber, int Start, int Amount, byte[] Data) {
        int Address;
        int NumElements;
        int MaxElements;
        int TotElements;
        ...
        while ((TotElements > 0) && (LastError == 0)) {
            ...
            ///////////////////////=============
System.out.printf(
    "READ SEND: Len=%d TPKT=%d ParLen=%d DataLen=%d Seq=%d%n",
    Size_RD,
    S7.GetWordAt(PDU, 2),
    S7.GetWordAt(PDU, 13),
    S7.GetWordAt(PDU, 15),
...
for (int i = 0; i < Size_RD; i++) {
    System.out.printf("%02X ", PDU[i] & 0xFF);
}
System.out.println();
            /////////////////////////===========
    ...
    synchronized public int WriteArea(int Area, int DBNumber, int Start, int Amount, byte[] Data) {
        int Address;
        int NumElements;
        int MaxElements;
        int TotElements;
